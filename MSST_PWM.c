...
void Epwm1Init()
{
    ...
    EPwm1Regs.TBCTL.bit.SYNCOSEL = 0; // Sync when TBCTR = 0
...
void PwmInit()
{
    ...
    SyncSocRegs.SYNCSELECT.bit.EPWM7SYNCIN = 5; // EPWM7 Sync select: Extsync 1
    EDIS;
    ECapInit();
    Epwm1Init();
    Epwm2Init();
...
void Dab_Update()
{
    dab_prd = (Uint16)((ECap1Regs.CAP2 + ECap1Regs.CAP4) >> 2) + 1;
    dab_cmp = dab_prd >> 1;
    EPwm7Regs.TBPRD = dab_prd;
    EPwm7Regs.CMPA.bit.CMPA = dab_cmp;
    EPwm8Regs.TBPRD = dab_prd;
    EPwm8Regs.CMPA.bit.CMPA = dab_cmp;

    if(dab_phs >= 0)
    {
        EPwm7Regs.TBPHS.bit.TBPHS = dab_phs;
        EPwm8Regs.TBPHS.bit.TBPHS = dab_phs;
